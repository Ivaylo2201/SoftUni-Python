...
# The opposite of filter. In this case where nationality is not null
def find_authors_nationalities() -> str:
    return '\n'.join(
        f"{str(author)} is {author.nationality}" for author in Author.objects.exclude(
...
def order_books_by_year() -> str:
    return '\n'.join(
        f"{book.publication_year} year: {str(book)}"
        for book in Book.objects.order_by('publication_year', 'title')
    )


def delete_review_by_id(review_id: int) -> str:
    reviewer_name: str = Review.objects.get(id=review_id).reviewer_name
...
def filter_authors_by_nationalities(nationality: str) -> str:
    return '\n'.join(
        author.biography if author.biography else str(author)
...
def filter_authors_by_birth_year(first_year: int, second_year: int) -> str:
    return '\n'.join(
        f"{author.birth_date}: {str(author)}"
        for author in Author.objects.filter(
...
def change_reviewer_name(reviewer_name: str, reviewer_new_name) -> QuerySet:
