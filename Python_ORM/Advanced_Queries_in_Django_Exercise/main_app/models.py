...
class Invoice(models.Model):
    ...
    @classmethod  # We can use @staticmethod here with Invoice.objects... or with self instead of cls
...
class Programmer(models.Model):
    ...
    # Take all projects of the programmer and connect them to the used technologies
...
class Task(models.Model):
    ...
    def completed_mid_priority_tasks(cls) -> QuerySet:
        # Getting the tasks that are with medium priority and completed
        return cls.objects.filter(
            Q(priority='Medium') &
            Q(is_completed=True)
        )
    ...
    def search_tasks(cls, query: str) -> QuerySet:
        # The query is contained either in the title, either in the description
        return cls.objects.filter(
            Q(title__icontains=query) |
            Q(description__icontains=query)
        )

    # When using self -> we hard code a value (the attribute with self)
    # When using F -> dynamically change value according to the object
